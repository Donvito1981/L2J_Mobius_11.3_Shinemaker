...
public class ExAcquireAPSkillList extends ServerPacket
{
	...
	public void write()
	{
		...
		writeByte(_enable); // 430 Success changed to Result.
		if (_player.isDualClassActive())
		{
			writeInt(_remainingAbilityPointsAPresetDual); // APresetRemainAP
			writeInt(_remainingAbilityPointsBPresetDual); // BPresetRemainAP
		}
		else
		{
			writeInt(_remainingAbilityPointsAPresetMain); // APresetRemainAP
			writeInt(_remainingAbilityPointsBPresetMain); // BPresetRemainAP
		}
		writeByte(_abilityPreset); // CurrentPreset
